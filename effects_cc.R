...
# joint probabilites of counterfactuals of U given sensitivity param rho
...
tau_mu_rho25 <- compute_rmst(wdat,M_args=list(gr=0,ref=F,score=factor(rep(0:2,each=3))),gr=1,score=factor(rep(0:2,3)),change=T,
...
tau_mu_rho75 <- compute_rmst(wdat,M_args=list(gr=0,ref=F,score=factor(rep(0:2,each=3))),gr=1,score=factor(rep(0:2,3)),change=T,
                             params=params,dat0=dat0,arg_list=argL,
                             prU=pr_uj75%>%rename("score_m":=u0,"score":=u1),prW=pr_w)
tau_mu_rho0 <- compute_rmst(wdat,M_args=list(gr=0,ref=F,score=factor(rep(0:2,each=3))),gr=1,score=factor(rep(0:2,3)),change=T,
                            params=params,dat0=dat0,arg_list=argL,
                             prU=pr_uj00%>%rename("score_m":=u0,"score":=u1),prW=pr_w)
tau_mu_rho100 <- compute_rmst(wdat,M_args=list(gr=0,ref=F,score=factor(rep(0:2,each=3))),gr=1,score=factor(rep(0:2,3)),change=T,
                              params=params,dat0=dat0,arg_list=argL,
...
eff_de_ie <- rbind(
  data.frame(eff="IE",
           assumption=c("rho0","rho25","rho50","rho75","rho100"),
